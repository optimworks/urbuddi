...
public class ResourceTrackingTestCase {
	...
	String employeeID = data.get(3)[0];
	String allocateddate = data.get(2)[0];
	String serialid = data.get(2)[1];
	String model = data.get(2)[2];
	String resourcedropdown = data.get(2)[4];

	@TestData(columnNames = "Username,Password,, employeeID, allocateddate, serialid, model,resourcedropdown")

	@Qualifier
	public String qualifier() {
		return " - " + " Username = " + username + " and " + " password = " + password + " EmployeeID = " + employeeID
				+ " allocateddate	= " + allocateddate + " serialid	= " + serialid + " model= " + model+ " resourcedropdown= " + resourcedropdown;
	}
	...
	public void resourceTrackingAddResource() {
		...
		String randomserialid = Integer.toString(randomNumber);
		urbuddy.clickResourceTrackingMenuOption();
		urbuddy.verifyResourceTrackingPage();
		urbuddy.addResource(employeeID, allocateddate, randomserialid, model,resourcedropdown);
	}
}
